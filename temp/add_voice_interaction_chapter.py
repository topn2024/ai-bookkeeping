# -*- coding: utf-8 -*-
"""
将第15章的对话式记账助手(15.7)保留原位置，
并在15.11之后添加全新的15.12智能语音交互系统章节
"""

import re

def add_voice_interaction_chapter():
    with open('D:/code/ai-bookkeeping/docs/design/app_v2_design.md', 'r', encoding='utf-8') as f:
        content = f.read()

    # 新的15.12章节内容 - 使用原始字符串避免转义问题
    new_chapter = r'''
...
class IntentRecognitionEngine {
  ...
  VoiceIntentType _parseIntentType(String name) {
...
class VoiceBookkeepingService {
  ...
  Future<VoiceBookkeepingResult> _processBatchRecord(
    ...
    return VoiceBookkeepingResult.batchConfirmation(
      ...
      summary: '识别到${processedTransactions.length}笔交易，'
               '共计${processedTransactions.fold<double>(0, (sum, tx) => sum + tx.amount).toStringAsFixed(2)}元',
    );
  }
  ...
  String _generatePromptForMissingField(String field) {
    switch (field) {
      case 'amount':
        return '请问这笔消费是多少钱？';
      case 'category':
        return '请选择一个分类，或说出分类名称';
      case 'account':
        return '从哪个账户支出？';
      default:
        return '请补充 $field 信息';
...
class VoiceConfigurationService {
  ...
  static const Map<String, ConfigurableItem> _configurableItems = {
    ...
    '月预算': ConfigurableItem(type: ConfigType.budget, categoryKey: 'monthly'),
  ...
  Future<VoiceConfigResult> _processBudgetConfig(
    ...
    if (newAmount == null || newAmount <= 0) {
      return VoiceConfigResult.needMoreInfo(
        prompt: '请问您要将${targetBudget}设为多少？',
    ...
    return VoiceConfigResult.needConfirmation(
      ...
      newValue: newAmount,
      summary: '将$targetBudget从${currentBudget?.amount ?? 0}元修改为${newAmount}元',
    );
  }
  ...
  /// 处理通用设置
  Future<VoiceConfigResult> _processGeneralConfig(
    String voiceText,
    VoiceIntent intent,
  ) async {
    // 主题切换
    if (voiceText.contains('主题') || voiceText.contains('深色') || voiceText.contains('浅色')) {
      String? theme;
      if (voiceText.contains('深色') || voiceText.contains('暗色') || voiceText.contains('黑色')) {
        theme = 'dark';
      } else if (voiceText.contains('浅色') || voiceText.contains('亮色') || voiceText.contains('白色')) {
        theme = 'light';
      } else if (voiceText.contains('跟随系统') || voiceText.contains('自动')) {
        theme = 'system';
      }
      ...
      if (theme != null) {
        return VoiceConfigResult.needConfirmation(
          ...
          targetKey: 'theme',
          newValue: theme,
          summary: '切换到${theme == 'dark' ? '深色' : theme == 'light' ? '浅色' : '跟随系统'}主题',
        );
      }
    }

    // 语言切换
    if (voiceText.contains('语言') || voiceText.contains('英文') || voiceText.contains('中文')) {
      String? language;
      if (voiceText.contains('英文') || voiceText.contains('English')) {
        language = 'en';
      } else if (voiceText.contains('中文') || voiceText.contains('简体')) {
        language = 'zh';
      }
      ...
      if (language != null) {
        return VoiceConfigResult.needConfirmation(
          ...
          targetKey: 'language',
          newValue: language,
          summary: '切换语言为${language == 'en' ? '英文' : '中文'}',
        );
      }
    }

    return VoiceConfigResult.error('暂不支持该设置的语音配置');
  }

  /// 确认并执行配置
  Future<VoiceConfigResult> confirmAndExecute(VoiceConfigResult pendingConfig) async {
    try {
      switch (pendingConfig.configType) {
        case ConfigType.budget:
          await _executeBudgetConfig(pendingConfig);
          break;
        case ConfigType.category:
          await _executeCategoryConfig(pendingConfig);
          break;
        case ConfigType.reminder:
          await _executeReminderConfig(pendingConfig);
          break;
        case ConfigType.vault:
          await _executeVaultConfig(pendingConfig);
          break;
        case ConfigType.general:
          await _executeGeneralConfig(pendingConfig);
          break;
      }

      return VoiceConfigResult.success(
        message: '${pendingConfig.summary}，已完成',
      );
    } catch (e) {
      return VoiceConfigResult.error('配置失败：$e');
    }
  }
  ...
  String? _extractVaultName(String text) {
    final patterns = [
      RegExp(r'[创建新建添加](?:一个)?["\'「]?([^\s"\'「」]+)["\'」]?小金库'),
      RegExp(r'([^\s]+)小金库'),
    ];
...
class VoiceNavigationService {
  ...
  static const Map<String, NavigationTarget> _navigationTargets = {
    ...
    '账���': NavigationTarget(route: '/accounts', displayName: '账户管理'),
  ...
  Future<VoiceNavigationResult> _processDirectNavigation(String voiceText) async {
    ...
    // 2. 模糊匹配（使用LLM）
    final llmResult = await _fuzzyMatchNavigation(voiceText);
    if (llmResult != null) {
      return VoiceNavigationResult.navigate(
        target: llmResult,
        message: '正在打开${llmResult.displayName}',
      );
    }
  ...
  Future<VoiceNavigationResult> _processFeatureSearch(String voiceText) async {
    ...
    for (final entry in _featureKeywords.entries) {
      for (final keyword in entry.value) {
        if (voiceText.contains(keyword) || _isSimilar(voiceText, keyword)) {
    ...
    // 2. 使用LLM理解用户意图并推荐
    final recommendation = await _llmRecommendFeature(voiceText);
    if (recommendation != null) {
      return VoiceNavigationResult.navigateWithGuide(
        target: recommendation.target,
        message: recommendation.message,
        guide: recommendation.guide,
      );
    }
  ...
  Future<VoiceNavigationResult> _processQuickAction(String voiceText) async {
    ...
    if (voiceText.contains('拍照') || voiceText.contains('拍一下') || voiceText.contains('相机')) {
      return VoiceNavigationResult.quickAction(
    ...
    if (voiceText.contains('语音') || voiceText.contains('说')) {
      return VoiceNavigationResult.quickAction(
  ...
  /// 执行导航
  Future<void> executeNavigation(NavigationTarget target) async {
    await _navigationService.navigateTo(target.route);
  }
  ...
  bool _isSimilar(String text1, String text2) {
    ...
    return common / words2.length > 0.6;
  }
...
class VoiceQueryService {
  ...
  /// 处理收入查询
  Future<VoiceQueryResult> _processIncomeQuery(
    String voiceText,
    VoiceIntent intent,
  ) async {
    final timeRange = _parseTimeRange(voiceText);

    final incomes = await _transactionRepo.getIncomes(
      startDate: timeRange.start,
      endDate: timeRange.end,
    );

    final totalAmount = incomes.fold<double>(0, (sum, tx) => sum + tx.amount);

    return VoiceQueryResult.success(
      summary: '${timeRange.displayName}，总收入${totalAmount.toStringAsFixed(2)}元，共${incomes.length}笔',
      data: QueryData(
        type: QueryDataType.income,
        amount: totalAmount,
  ...
  Future<VoiceQueryResult> _processTrendQuery(
    ...
    return VoiceQueryResult.success(
      ...
      data: QueryData(
        type: QueryDataType.trend,
        currentAmount: thisMonth,
        previousAmount: lastMonth,
  ...
  /// 处理余额查询
  Future<VoiceQueryResult> _processBalanceQuery(
    String voiceText,
    VoiceIntent intent,
  ) async {
    final accounts = await _transactionRepo.getAllAccountsWithBalance();
    final totalBalance = accounts.fold<double>(0, (sum, a) => sum + a.balance);

    String summary = '总余额${totalBalance.toStringAsFixed(2)}元';

    // 列出主要账户
    if (accounts.length <= 3) {
      final details = accounts.map((a) => '${a.name}${a.balance.toStringAsFixed(2)}元').join('，');
      summary += '。$details';
    }
  ...
  /// 处理报告查询
  Future<VoiceQueryResult> _processReportQuery(
    String voiceText,
    VoiceIntent intent,
  ) async {
    // 判断报告类型
    if (voiceText.contains('月报') || voiceText.contains('本月')) {
      return await _generateMonthlyReport();
    } else if (voiceText.contains('周报') || voiceText.contains('本周')) {
      return await _generateWeeklyReport();
    } else if (voiceText.contains('年报') || voiceText.contains('今年')) {
      return await _generateYearlyReport();
    }
  ...
  Future<VoiceQueryResult> _generateMonthlyReport() async {
    final stats = await _statsService.getMonthlyStats();

    // 使用LLM生成自然语言报告
    final report = await _llmService.generateReport(
      type: 'monthly',
      data: stats.toJson(),
    );
    ...
    return VoiceQueryResult.success(
      summary: report,
      data: QueryData(
        type: QueryDataType.report,
        reportData: stats,
      ),
      followUpSuggestions: ['查看详细报表', '对���上月'],
    );
  }
...
class VoiceSessionManager {
  ...
  Future<VoiceResponse> processVoiceInput(String voiceText) async {
    ...
    if (intent.type == VoiceIntentType.confirm) {
      return _handleConfirm();
    }
    if (intent.type == VoiceIntentType.help) {
      return _handleHelp();
    }
  ...
  /// 处理上下文相关输入（多轮对话）
  Future<VoiceResponse> _handleContextualInput(String voiceText) async {
    switch (_state) {
      case VoiceSessionState.waitingAmount:
        return await _handleAmountInput(voiceText);

      case VoiceSessionState.waitingCategory:
        return await _handleCategoryInput(voiceText);

      case VoiceSessionState.waitingConfirmation:
        return await _handleConfirmationInput(voiceText);

      case VoiceSessionState.waitingConfigValue:
        return await _handleConfigValueInput(voiceText);
  ...
  Future<VoiceResponse> _dispatchIntent(String voiceText, VoiceIntent intent) async {
    ...
    // 闲聊
    if (intent.type == VoiceIntentType.chat) {
      return await _handleChat(voiceText);
    }
  ...
  /// 处理金额输入
  Future<VoiceResponse> _handleAmountInput(String voiceText) async {
    final amount = _extractAmount(voiceText);

    if (amount == null || amount <= 0) {
      return VoiceResponse(
        message: '请说一个有效的金额，比如"50元"或"一百块"',
        state: VoiceResponseState.needInput,
      );
    }

    // 更新上下文中的交易
    final transaction = (_context?.pendingTransaction as Transaction).copyWith(
      amount: amount,
    );
    _context = _context?.copyWith(pendingTransaction: transaction);

    // 检查是否还需要其他信息
    if (transaction.categoryId == null) {
      _state = VoiceSessionState.waitingCategory;
      return VoiceResponse(
        message: '好的，${amount}元。请选择一个分类：',
        suggestions: ['餐饮', '交通', '购物', '娱乐', '其他'],
        state: VoiceResponseState.needInput,
      );
    }

    // 请求确认
    _state = VoiceSessionState.waitingConfirmation;
    return VoiceResponse(
      message: '确认记录：${transaction.categoryName} ${transaction.amount}元？',
      suggestions: ['确认', '修改', '取消'],
      state: VoiceResponseState.needConfirmation,
    );
  }

  /// 处理分类选择
  Future<VoiceResponse> _handleCategoryInput(String voiceText) async {
    // 匹配分类
    final category = await _matchCategory(voiceText);

    if (category == null) {
      return VoiceResponse(
        message: '没有找到这个分类，请选择：',
        suggestions: ['餐饮', '交通', '购物', '娱乐', '其他'],
        state: VoiceResponseState.needInput,
      );
    }
    ...
    _state = VoiceSessionState.waitingConfirmation;
    return VoiceResponse(
      message: '确认记录：${category.name} ${transaction.amount}元？',
  ...
  VoiceResponse _handleCancel() {
    ...
    return VoiceResponse(
      message: '已取消',
      state: VoiceResponseState.complete,
    );
  }
...
enum VoiceSessionState {
  ...
  waitingConfigValue,     // 等待配置值
  waitingNavChoice,       // 等待导航选择
}

/// 语音响应状态
enum VoiceResponseState {
  complete,           // 完成
  needInput,          // 需要更多输入
...
'''
    ...
    # 找到15.11节末尾和第16章开头之间的位置
    # 在 "## 16. 性能设计与优化" 之前插入新章节
    marker = "---\n\n## 16. 性能设计与优化"

    if marker in content:
        # 在第16章之前插入
        content = content.replace(marker, new_chapter + "\n---\n\n## 16. 性能设计与优化")
        print("成功在第16章之前插入15.12节")
    else:
        # 尝试另一种方式查找
        alt_marker = "## 16. 性能设计与优化"
        if alt_marker in content:
            # 找到15.11结束的位置（在```后面）
            pos_16 = content.find(alt_marker)
            # 向前找最近的 --- 或段落分隔
            insert_pos = content.rfind("---", 0, pos_16)
            if insert_pos != -1:
                content = content[:insert_pos] + new_chapter + "\n" + content[insert_pos:]
                print("成功插入15.12节（使用替代方案）")
            else:
                content = content.replace(alt_marker, new_chapter + "\n\n" + alt_marker)
                print("成功插入15.12节（直接在16章前）")
        else:
            print("错误：未找到插入位置")
            return
    ...
    print("完成！已添加15.12智能语音交互系统章节")

if __name__ == '__main__':
    add_voice_interaction_chapter()
