...
new_paragraphs = {
    ...
    '[0004] 现有技术二（会计领域FIFO成本法）：先进先出（First In First Out，FIFO）成本核算方法是美国公认会计准则（US GAAP）和国际财务报告准则（IFRS）认可的标准存货计价方法，广泛应用于证券成本核算和库存管理。技术缺陷：（1）仅用于成本计算和税务核算，不计算资产的持有时长；（2）各系统实现不统一，缺乏标准化数据模型；（3）无通用框架支持资金、股票、库存等多种资源类型的统一处理。',
    ...
    '[0005] 现有技术三（个人记账软件）：市场上现有的个人记账应用提供收支分类和统计功能。技术缺陷：（1）仅支持简单的收支分类和汇总统计；（2）采用月度或周度汇总，统计粒度过粗；（3）无时间维度的资金分析功能，无法建立收入与支出之间的对应关系，无法回答"这笔消费花的是哪天的收入"这一问题。',
    ...
    '[0007] 现有技术五（财务健康评估工具）：市场上存在基于问卷调查的财务健康评估工具。技术缺陷：（1）基于主观问卷而非客观交易数据；（2）评估维度单一，无时间维度的资金分析功能；（3）无法追踪具体的资金流动路径。',
    ...
    '[0008] 综上所述，现有技术存在以下共性技术问题：（1）无法建立收入与支出之间的精确对应关系；（2）缺乏资金持有时长的量化计算方法；（3）修改历史数据后效率低下；（4）缺乏标准化的数据模型；（5）未将FIFO模型应用于个人资金时间价值计算领域；（6）无法扩展至股票、库存等变体场景。',
...
distinction_updates = {
    ...
    '[0011] 区别特征二（vs 会计FIFO成本法）：本发明在成本核算基础上增加持有时长计算，采用消耗金额加权平均算法：资金时间价值 = Σ(消耗金额ᵢ×天数ᵢ) / Σ(消耗金额ᵢ)；提供双向追溯查询和消费链路可视化（桑基图）。',
    ...
    '[0013] 区别特征四（vs 区块链追踪）：本发明采用轻量级本地存储，查询延迟低；支持离线计算和增量更新，无需网络确认；提供通用框架支持资金、股票、库存等多种资源类型。',
}
...
for i, para in enumerate(doc.paragraphs):
    ...
    # 检查背景技术段落
    for old_text, new_text in new_paragraphs.items():
        if text == old_text:
            if new_text == '':
                para.clear()
                deleted_indices.append(i)
                print(f'已删除: [{i}] {old_text[:40]}...')
            else:
                para.clear()
                para.add_run(new_text)
                print(f'已修改: [{i}] {old_text[:40]}...')
            modified_count += 1
            break

    # 检查区别特征段落
    for old_text, new_text in distinction_updates.items():
        if text == old_text:
            para.clear()
            para.add_run(new_text)
            print(f'已修改区别特征: [{i}] {old_text[:40]}...')
            modified_count += 1
            break
