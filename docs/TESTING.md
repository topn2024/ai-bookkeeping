...
| 账户类型标签 | 验证现金/银行卡等标签 |
| 账户类型图标 | 验证账户类型图标 |
| 交易条目长按显示 | 验证长按交易条目显示编辑删除按钮 |
| 交易条目左滑显示 | 验证左滑交易条目显示编辑删除按钮 |
| 交易条目右滑收起 | 验证右滑收起已展开的按钮 |
| 编辑按钮点击 | 验证点击编辑按钮触发编辑回调 |
| 删除按钮点击 | 验证点击删除按钮触发删除回调 |
| 震动反馈 | 验证长按/操作时触发震动反馈 |
| 滚动自动收起 | 验证列表滚动时自动收起展开的按钮 |
| 单一激活状态 | 验证同时只有一个条目处于激活状态 |
