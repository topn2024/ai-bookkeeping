...
> 更新日期：2026-01-09
> 文档状态：正式版
> 对应设计方案：[app_v2_design.md](./design/app_v2_design.md)（35章完整版）
...
| 维度 | 策略 | 说明 |
|------|------|------|
| **核心定位** | 全免费应用 + AI增值服务 | 所有功能免费使用，无付费墙 |
| **用户增长** | 降低使用门槛 | 最大化用户获取 |
| **AI激励** | AI功能需登录 | 提高用户注册转化 |

### 17.2 用户类型

| 用户类型 | 描述 | 功能权限 |
|----------|------|----------|
| **访客用户 (guest)** | 无需注册即可使用 | 所有本地功能，数据仅存储在本地 |
| **登录用户 (loggedIn)** | 已注册并登录的用户 | 所有功能（含AI），支持云端同步和多设备 |

### 17.3 功能分类

| 分类 | 说明 | 用户要求 | 技术特征 |
|------|------|----------|----------|
| **basic** | 基础功能 | 访客可用 | 本地计算，无需网络API |
| **aiPowered** | AI功能 | 仅登录用户 | 需调用大模型API |

### 17.4 访客可用功能（基础功能）

| 模块 | 功能项 | 说明 |
|------|--------|------|
| **记账核心** | 手动记账、交易列表、交易搜索、账户管理、分类管理 | 完整记账流程 |
| **预算系统** | 手动预算、预算追踪、零基预算、小金库 | 预算管理全功能 |
| **钱龄系统** | 钱龄计算、钱龄趋势分析 | FIFO/LIFO/加权平均 |
| **统计分析** | 收支统计报表、数据可视化图表 | 本地分析 |
| **数据管理** | 数据导入（CSV/Excel）、数据导出备份 | 本地数据管理 |
| **其他功能** | 位置服务、习惯统计、成就系统、家庭账本、提醒通知、主题设置 | 辅助功能 |

### 17.5 登录用户专属功能（AI功能）

| 模块 | 功能项 | 说明 |
|------|--------|------|
| **AI输入识别** | 语音记账、图像识别、OCR识别、账单智能解析 | 通义千问ASR/OCR |
| **AI语义理解** | 自然语言意图理解、对话式交互、多笔交易智能拆分 | 大模型语义处理 |
| **AI智能建议** | 智能分类、预算建议、消费洞察、趋势预测 | AI分析能力 |
| **AI增强功能** | 异常检测、自学习、语音播报、协同学习 | 云端AI服务 |
| **云端服务** | 云端同步、多设备数据共享 | 账户关联服务 |

### 17.6 登录引导策略

| 场景 | 引导文案 | 操作按钮 |
|------|----------|----------|
| 点击语音按钮 | "登录后即可使用AI语音记账，说话就能记账" | [登录] [继续手动记账] |
| 点击拍照按钮 | "登录后即可拍照自动识别，票据秒变账单" | [登录] [继续手动记账] |
| 对话功能 | "登录后即可与AI助手对话，查询更便捷" | [登录] [取消] |
| 智能建议 | "登录后获取AI个性化建议，理财更智能" | [登录] [暂不需要] |

### 17.7 设计原则

| 原则 | 说明 | 实现方式 |
|------|------|----------|
| **功能完整性** | 访客也能完成完整记账流程 | 核心记账功能不设限制 |
| **体验一致性** | 访客与登录用户UI体验一致 | 仅在触发AI功能时提示登录 |
| **转化自然性** | 登录引导自然不打断 | 在用户需要AI时才提示 |
| **价值清晰性** | 登录价值清晰可感知 | 明确展示AI功能的便利性 |
...
| **第七部分：用户增长** | 30-31章 | NPS提升、低成本获客 |
| **第八部分：高可用与扩展** | 32-34章 | 高可用架构(10万DAU)、分布式一致性、可扩展性增强(100万DAU预留) |
| **第九部分：用户体系** | 35章 | 用户体系与权限设计（访客/登录用户分级、AI功能授权） ⭐新增 |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 2.0.9 | 2026-01-09 | **第35章用户体系与权限设计同步**：新增第十七章「用户体系与权限设计」完整内容；商业模式定位（全免费应用+AI增值服务）；用户类型定义（访客/登录用户两级）；功能分类（basic基础功能/aiPowered AI功能）；访客可用功能清单（记账核心、预算系统、钱龄系统、统计分析、数据管理）；登录用户专属功能清单（AI输入识别、AI语义理解、AI智能建议、AI增强功能、云端服务）；登录引导策略（4种触发场景和引导话术）；设计原则（功能完整性、体验一致性、转化自然性、价值清晰性）；附录A新增「用户与权限模块」；附录C章节对照表从33章扩展至35章；新增第八部分（高可用与扩展32-34章）和第九部分（用户体系35章） |
