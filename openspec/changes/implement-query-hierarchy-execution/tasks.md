...
- [ ] 设计统一的查询请求数据结构（QueryRequest）
- [ ] 设计统一的查询响应数据结构（QueryResponse）
- [ ] 实现查询复杂度判定引擎（QueryComplexityAnalyzer）
  - [ ] 实现时间跨度评分算法
  - [ ] 实现数据维度评分算法
  - [ ] 实现数据点数评分算法
  - [ ] 实现查询类型评分算法
  - [ ] 实现层级判定逻辑
- [ ] 实现查询结果路由器（QueryResultRouter）
- [ ] 编写单元测试（复杂度判定）
...
- [ ] 设计轻量卡片UI组件（LightweightQueryCard）
- [ ] 实现进度条可视化
- [ ] 实现占比可视化
- [ ] 实现对比可视化（两期对比）
- [ ] 实现自动淡出动画（3秒）
...
- [ ] 添加fl_chart依赖到pubspec.yaml
- [ ] 设计交互图表组件（InteractiveQueryChart）
- [ ] 实现折线图（趋势分析）
- [ ] 实现柱状图（分类对比）
- [ ] 实现饼图（占比分布）
- [ ] 实现图表交互功能（点击查看明细）
