...
class ExpenseTarget {
  ...
  factory ExpenseTarget.fromMap(Map<String, dynamic> map) {
    return ExpenseTarget(
      ...
      month: ((map['month'] as int?) ?? 1).clamp(1, 12),
      icon: IconData(map['icon_code'] as int? ?? 0xe8d4, fontFamily: 'MaterialIcons'),
      color: Color(map['color_value'] as int? ?? 0xFF4CAF50),
      alertThreshold: ((map['alert_threshold'] as int?) ?? 80).clamp(0, 100),
      ...
      isExceeded: map['is_exceeded'] as bool? ?? false,
      isNearLimit: map['is_near_limit'] as bool? ?? false,
    );
  }
}
